...
function repoFromUrl(url: string): string {
  ...
  if (!m) throw new Error(`cannot parse GitHub repo from remote URL: ${url}`);
  return m[1];
}

export function fork(opts: ForkOptions): void {
  const { upstream } = opts;
  ...
  if (existsSync(layer)) throw new Error(`${layer} already exists in the source repo`);
  for (const name of workflowFiles) {
    if (existsSync(join(workflows, name))) throw new Error(`${join(workflows, name)} already exists in the source repo`);
  }
  ...
  writeFileSync(
    join(workflows, "forker-update.yml"),
    readFileSync(join(TEMPLATES, "workflows", "forker-update.yml"), "utf8").replaceAll("{{CRON}}", cron),
  );
