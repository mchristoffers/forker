...
- one **project-specific** script, `forker/build.sh`, generated by an AI agent (Claude Code or Codex) through the `create-build` skill (called by the `forker` skill), and fixed by `repair-build`
...
flowchart TD
  ...
  X[make build broke after make update] --> R[agent invokes repair-build skill]
