...
- one **project-specific** script, `forker/build.sh`, generated by an AI agent (Claude Code or Codex) through the `create-or-repair-build` skill (called by the `forker` skill)
...
flowchart TD
  ...
  F --> CR[agent invokes create-or-repair-build skill]
  CR --> I[inspect source: CI workflows, README, manifests, upstream releases]
  I --> W[write forker/build.sh]
  W --> B{make build ok and forker/dist has artifacts?}
  B -- no --> W
  B -- yes --> C[commit + push build.sh]
  X[make build broke after make update] --> R[agent invokes create-or-repair-build: repair mode]
